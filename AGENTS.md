...
| (copyperm-grants) `WithDifferentNames$`, `AttachedTo$` and `Chooser$` remain unread on `api:CopyPermanent`; the copy otherwise carries its `AddTriggers$`, `AddSVars$`, `AddAbilities$` and `ImprintTokens$` riders. | `effects/copypermanent.go` (`effCopyPermanent`) | M4 (the remaining name, attachment and chooser riders) |
