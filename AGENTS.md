...
| The shared unless gate pays mana from the floating pool only (no mid-resolution tap-to-pay). An unless-cost the strict parser cannot price -- an unfolded `X`/`XX`, `PayEnergy<N>`, `Return<...>`, `LifeTotalHalfUp`, `DefinedCost_*`, `Y`/`Z`, and an SVar body the shared count evaluator cannot resolve -- is a HARD DECLINE: the ask is posed and recorded but cannot be answered "pay". A resolvable SVar body on a Counter or CopySpellAbility unless-cost is now folded to a concrete generic amount (Mausoleum Wanderer's `Sacrificed$CardPower`); every other API's SVar stays a hard decline, and exotic selector bindings remain unresolved. | `effects/unless.go` (`unlessProceed`, `poseUnlessAsk`, `UnlessCostResolved`), `rules/resolution.go` (`unless_pay` arm, `ParseUnlessCost`) | M4 (X-cost grammar, tap-to-pay, remaining selector bindings, the other APIs' SVar folds) |
