...
| (dig1) `Optional$` on the no-choice path, the primary `LibraryPosition$`, `Choser$` and `DigNum$ X` remain unread; a multi-player `Dig` asks only its FIRST strict-superset library a take (later libraries take deterministically) but each library poses its own bottom arrange (the `Ctx.ArrangeTarget` cursor resumes later targets after a suspension). The default remainder -- no `DestinationZone2$`/`SkipReorder$`, measured 635 of the corpus's 809 `SP$` and `DB$` Dig lines (137 of them also carrying the unread `RestRandomOrder$`) -- goes to the BOTTOM in an order the player picks: a Min==Max==N `KArrange` (`dig_bottom`, answer order = bottom order) when two or more cards remain, a no-ask deterministic move for one, the offered order for a no-host (R-9). | `effects/cardflow.go` (`effDig`), `effects/zone.go` (`effHiddenPick`, `effSearchLibrary`), `effects/play.go` (`effPlay`), `rules/resolution.go` (the "dig"/"dig_arrange" arms), `rules/arrange.go` | M4 (per-library take asks; optional no-choice path; primary `LibraryPosition$`; `Choser$`; `DigNum$ X`; `RestRandomOrder$`) |
