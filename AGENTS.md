...
| (api:Clone) The ETB-copy route offers only bodies whose parameter set is inside a POSITIVE whitelist — {Choices$, AddTypes$, AddKeywords$, SpellDescription$}, 25 of 66 corpus carriers (census: `rules/etb_clone_whitelist_census_test.go`) — asks before entry, records the selected permanent, applies a replay-safe layer-1 copy with AddTypes/AddKeywords riders, and reverses that selection on a CR 733.1 cast abort. Non-cast entries and out-of-scope carriers (Vesuva's IntoPlayTapped$, Cursed Mirror's Duration$) keep the loud unimplemented-API Clone fallback; mid-resolution Clone still uses its deterministic Choices$ stand-in. | `rules/cast.go` (`collectETBChoices`, `etbOptions`, `etbAnswer`, `abortCast`, `etbCloneWhitelist`), `rules/replacement.go` (`replCtx`), `effects/clone.go` (`effClone`), `events/apply.go` (`Choose`) | M4 (non-cast ETB asks, remaining Clone riders and a real mid-resolution Choices$ ask) |
