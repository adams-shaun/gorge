...
| `Effect` registers real continuous effects only for `CantTarget`, `CantRegenerate` and the cost-modifier modes; `Triggers$` effects and every other `StaticAbilities$` mode stay a bare Note. Explicit exotic `Duration$` forms remain incomplete, but an absent `Duration$` now correctly expires at cleanup for every source kind. | `effects/misc.go` (`effEffect`, `effectUntilEOT`, `IsNextTurnDuration`), `rules/layers.go` (`restrictionApplies`, `AddContinuous`, `EndOfTurnCleanup`), `state/continuous.go` | M4 (Triggers$ effects, full Duration$ grammar, the other Effect-delivered grant keywords) |
