...
| `Discard` asks for `Mode$ RevealYouChoose` (Thoughtseize, Duress): the caster chooses from the DiscardValid$-filtered hand via a KModes mid-resolution ask, and `Mode$ RevealDiscardAll` (Cabal Therapy) discards every matching card. Remaining stand-ins: a no-ask host falls back to `hand[0]`, the cleanup-step discard stays a deterministic front-of-hand take, and `RevealYouChoose` with `NumCards$ > 1` asks once and uses the recorded answer without re-asking | `effects/cardflow.go` (`effDiscard`) | no longer a stand-in; see below |
