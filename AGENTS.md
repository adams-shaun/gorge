...
| Hidden-origin searches support private, owner-scoped `Origin$ Sideboard` cards and library searches; mixed hidden origins that include Hand or other unsupported chooser shapes remain narrowed to the existing deterministic path. | `effects/zone.go` (`effChangeZone`, `effSearchLibrary`) | M4 (remaining mixed-origin chooser grammar) |
