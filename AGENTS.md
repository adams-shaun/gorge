...
| (copyperm-grants) `WithDifferentNames$`, `AttachedTo$` and `Chooser$` remain unread on `api:CopyPermanent`; the copy otherwise carries its `AddTriggers$`, `AddSVars$`, `AddAbilities$` and `ImprintTokens$` riders, and the imprint read-back (`RememberObjects$ ImprintedLKI` on a DelTrig) resolves the minted tokens via `definedSpec`'s `ImprintedLKI` case. An unresolved grant name is one loud Note per call, never a silent drop. | `effects/copypermanent.go` (`effCopyPermanent`), `effects/context.go` (`imprintPileTargets`) | M4 (the remaining name, attachment and chooser riders) |
