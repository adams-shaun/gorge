...
| `effMana`'s no-host fallback still resolves `Produced$ Any`/`Combo Any` to colourless; the activation path asks for the colour. The bot-side production collector likewise counts `Any` as one colourless unit for its tap gate. The remaining Special selectors stay loud-fail: `EnchantedManaCost`, `DoubleManaInPool`, `EachColoredManaSymbol_Milled`, `EachColorAmong_ExiledWith`. | `effects/misc.go` (`effMana`), `rules/mana_activation.go`, `cards/mana_production.go` (`ProducedCounts`) | M4 (mana-choice decisions) |
| `RestrictValid$` is carried into the mana pool and enforced for `Spell`/`Activated` payment classes. The remaining unsupported classes and conditions (including `CostContainsX`, `CumulativeUpkeep`, `CantCast*`, `Static.*` and `nonSpell`) fail closed: 28 corpus carriers across 26 files are currently affected. | `effects/misc.go` (`effMana`), `rules/stack.go` (`restrictValidMatches`) | M4 (the remaining restriction grammar) |
