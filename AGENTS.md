...
| `Sacrifice` now reads `SacValid$` and, when aimed at a player, that player sacrifices a permanent chosen by a **deterministic engine-side stand-in** -- the first permanent in battlefield order satisfying `SacValid$` (default `Permanent`) -- rather than the player choosing in real Magic. A player with no matching permanent sacrifices nothing. The choice is a stand-in, not the real ask (the mid-resolution ask machinery is being reworked elsewhere); the remaining stand-in is that the player never actually chooses. On player-targeted lines the effect is also narrower than the card text in three ways the row must disclose: **`Amount$` is never read**, so the player always sacrifices exactly one permanent (measured: 56 defined-player-targeted lines carry an `Amount$` other than `1`, e.g. `2` x22, `X` x29, `SacX` x21, up to `13`); **`Optional$ True` is never read** (31 lines), so a "you *may* sacrifice" is now mandatory; and **`RememberSacrificed$` is read nowhere** (67 defined-player-targeted lines; the card text's "the permanent sacrificed" later reference is lost). **`ValidCard$` is also unread**: Expert-Level Safe (`DB$ Sacrifice | Defined$ You | ValidCard$ Card.Self`, "sacrifice this artifact") is the single corpus line that reaches the `Permanent` default, so with a land ahead of it in zone order its controller now sacrifices the **land** rather than the no-op the engine produced before this fix. `Sacrifice` on a concrete object target is unchanged (sacrificed as-is) and still ignores Indestructible and never consults the regeneration shield (sacrifice is not destruction) | `effects/zone.go` (`effSacrifice`) | M4 (the sacrifice choice is a real ask, not the deterministic first-match) |
