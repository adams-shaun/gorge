...
| `NameCard` asks over the embedder-supplied compiled corpus (`state.Game.NameUniverse`), filtered by the SA's own `ValidCards$`; with no corpus supplied it takes the deterministic pre-feature stand-in (the top of the caster's own library), and the sidecar's `NameUniverse` mode keeps a pre-feature log replaying with it. `ChooseFromList$`/`AtRandom$` are unread, so those forms offer the whole universe instead of the fixed/random list. `ValidDescription$` is Forge prompt text, not a filter; when `ValidCards$` is absent the builder maps the corpus's description forms to their predicate as a narrowly-safe fallback (no corpus NameCard carries one without `ValidCards$`). | `effects/namecard.go` (`NameChoices`, `descriptionSpec`), `effects/cardflow.go` (`effNameCard`), `rules/cast.go` (`etbOptions`), `rules/engine.go` (`Config.NameUniverse`), `host/persist.go` (`sidecar.NameUniverse`) | M4 (ChooseFromList$/AtRandom$ forms) |
