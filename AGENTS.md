...
| CopySpellAbility now asks for a new legal target when `MayChooseTarget$ True`; the inherited target is offered first and the no-host path keeps it. Remaining approximation: copies with multiple target declarations still use one flat target ask. | `rules/stack.go` (`AskCopyTargets`, `resolveTop`) | M4 (per-target-group copy choices) |
