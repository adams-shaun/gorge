...
| (copyperm-grants) `api:CopyPermanent` reads no granted-ability riders: `AddTriggers$`, `AddSVars$`, `AddAbilities$` and `WithDifferentNames$` each emit a loud Note and the copy is minted WITHOUT them, so Hofri Ghostforge's Spirit copy has no leaves-the-battlefield return ability and the card stays open. `ImprintTokens$ True` is not read on this path at all (Kharasha Foothills, Shredder, Shadow Master), so nothing records the copies for their delayed exile/sacrifice. `TokenRemembered$` IS read here and persisted on the mint, the same event-backed memory `api:Token` writes. | `effects/copypermanent.go` (`effCopyPermanent`, the unread-parameter Note block) | M4 (the granted-ability riders and `ImprintTokens$` on the copy path) |
