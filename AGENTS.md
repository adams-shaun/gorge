...
| (api:Clone) Whitelisted ETB `Choices$` copy replacements now ask before entry, record the selected permanent, and apply a replay-safe layer-1 copy with AddTypes/AddKeywords riders. Non-cast entries and Clone carriers with other riders remain loud/inert; mid-resolution Clone still uses its deterministic Choices$ stand-in. | `rules/cast.go` (`collectETBChoices`, `etbOptions`, `etbAnswer`), `rules/replacement.go` (`replCtx`), `effects/clone.go` (`effClone`), `events/apply.go` (`Choose`) | M4 (non-cast ETB asks, remaining Clone riders and a real mid-resolution Choices$ ask) |
