...
| (tokrepl1) Token-replacement stand-ins: a NON-chosen `Optional$ True` R: line (Flitwing Lyev) still takes the deterministic decline with no ask; `Type$ ReplaceController` and an unpriceable `Amount$` each emit a loud Note and leave the event verbatim; a chosen-copy body (`TokenScript$ Chosen`/`ValidChoices$`) that cannot ask while another decision owns the pipeline falls back to that same loud Note. NO CR 616.1 order choice (scan order); `effToken`'s riders land on the FIRST mint only, so replacement extras get none. | `rules/replacement.go` (`replacementEvent`, `continueCreateTokenReplacements`, `driveTokenReplacements`, `poseChosenTokenReplacement`, `applyTokenReplacementToPlan`, `tokenReplaceCount`) | M4 (ReplaceController, rider propagation to extras, the CR 616.1 order choice, cost-token provenance) |
