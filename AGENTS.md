...
| UnlessCost$ mana payments now open a one-source-at-a-time CR 601.2g mana-ability window after the payer chooses pay (the ward/attack-prop discipline); the pay option is offered only when floating mana plus `windowManaUnits` (the shared payment-window membership, which withholds InstantSpeed$/RestrictValid$ abilities and permanents with several free abilities) reaches the exact amount and colours, and no-host stays the deterministic decline. An unless-cost the strict parser cannot price -- an unfolded `X`/`XX`, `PayEnergy<N>`, `Return<...>`, `LifeTotalHalfUp`, `DefinedCost_*`, dynamic SVars, `Y`/`Z` -- is still a HARD DECLINE, and exotic selector bindings remain unresolved. | `effects/unless.go` (`poseUnlessAsk`), `rules/unless_payment.go` (`UnlessCostPayable`, `unlessManaBudget`, `askUnlessMana`), `rules/mana_available.go` (`windowManaUnits`), `rules/resolution.go` (`unless_pay` arm, `ParseUnlessCost`) | M4 (X-cost grammar, remaining selector bindings) |
