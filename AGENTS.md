...
| `NameCard` asks over the embedder-supplied compiled corpus (`state.Game.NameUniverse`), filtered by the SA's own `ValidCards$`; with no corpus supplied it names its own source (R-9). `ChooseFromList$`/`AtRandom$` are unread, so those forms offer the whole universe instead of the fixed/random list, and `ValidDescription$` is not parsed as a filter (no corpus NameCard carries it without `ValidCards$`). | `effects/namecard.go` (`NameChoices`), `effects/cardflow.go` (`effNameCard`), `rules/cast.go` (`etbOptions`), `rules/engine.go` (`Config.NameUniverse`) | M4 (ChooseFromList$/AtRandom$ forms) |
