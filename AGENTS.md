...
| UnlessCost$ mana payments open a one-source-at-a-time mana-ability window after the payer chooses pay; the pay option is offered only when floating mana plus the engine's reachable fixed land production satisfies the exact colours/amount, and no-host remains the deterministic decline. An unless-cost the strict parser cannot price -- an unfolded `X`/`XX`, `PayEnergy<N>`, `Return<...>`, `LifeTotalHalfUp`, `DefinedCost_*`, dynamic SVars, `Y`/`Z` -- is still a HARD DECLINE, and exotic selector bindings remain unresolved. | `effects/unless.go` (`poseUnlessAsk`), `rules/unless_payment.go` (`UnlessCostPayable`, `unless_mana`), `rules/resolution.go` (`unless_pay` arm, `ParseUnlessCost`) | M4 (X-cost grammar, remaining selector bindings) |
