...
| `Counter` now reads `UnlessCost$`: on the first pass the CONTROLLER OF THE COUNTERED SPELL (`c.Targets[0]`, per CR, deliberately NOT `effCopySpellAbility`'s `c.Controller` default — a counterspell charges its tax to the countered spell's controller, and the corpus counterspells carry no `UnlessPayer$`) is offered a KModes pay/decline decision recorded via the existing `ModeChosen`, and `rules/resolution.go`'s `unless_pay` arm pays it via `payMana`. What is NOT modelled: `payMana` spends only the FLOATING mana pool, with no tap-lands-to-pay flow mid-resolution, so a payer with untapped lands but an empty pool cannot actually pay and the engine records a decline (the spell is still countered — by far the common case); and a non-literal `UnlessCost$` (Grip of Amnesia's `ExileFromGrave<...>` graveyard-exile cost, Pact of Negation's upkeep tax) can never be paid, so it always declines. A no-ask host keeps the deterministic decline with a Note. | `effects/misc.go` (`effCounter`) | M4 (mana-choice decisions / tap-lands-to-pay) |
