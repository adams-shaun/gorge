...
**Row size is capped.** This file is loaded into every agent's context on
every turn; on 2026-09-22 the table had grown to 264 KB (one row per landed
ticket, up to 12 KB each) and was condensed to what it is now. A row is at
most three sentences in the Stand-in cell: what still deviates today, and any
decision a future implementer must honour. Measurements, corpus counts,
`Pinned by Test...` lists, review-round history and "what works" recaps go in
the commit message and the ticket report, never here. Edit the row you own
in place; do not append a second row for the same primitive. A reviewer
treats a row over that size as a MAJOR finding.
...
| `Mana` with `Produced$ Any` or `Combo Any` adds colourless instead of asking (Cavern of Souls, Chromatic Star, Lion's Eye Diamond, Lotus Petal). The remaining Special selectors stay loud-fail: `EnchantedManaCost`, `DoubleManaInPool`, `EachColoredManaSymbol_Milled`, `EachColorAmong_ExiledWith`. | `effects/misc.go` (`effMana`) | M4 |
| `RestrictValid$` spend restrictions are never read by `effMana` (Cavern of Souls' second mana ability, Eldrazi Temple's {C}{C}) -- colour and restriction both come out as plain colourless. | `effects/misc.go` (`effMana`) | M4 (mana restrictions) |
| A copy always keeps the original's target: `MayChooseTarget$` is read only to gate a Note, never to offer a chooser, so the copy's target selection is a stand-in (Chain Lightning's pay-copy clause). The missing target chooser, not an unread flag, is what remains. | `effects/copy.go` (`effCopySpellAbility`) | M4 |
...
| `RevealAllValid$` is unread: the reveal-all-matching family (`Break Expectations`, `Mind Spike`, `Thought Rattle`) still shows the FIRST card of the hand rather than every matching card, so the caster's chained pick chooses among one card. The plain hand-reveal pick itself is real (task infernaltutor1): a hand `Reveal`/`RevealHand` whose eligible pool exceeds its count (or carries `AnyNumber$`) asks the pool's owner which cards, an `Optional$` reveal asks its yes/no first, a DECLINED yes/no reveals nothing and poses no pick (fx45), and a pickable reveal over several `Defined$` players poses one pick per target (a per-target cursor, `RevealPickTarget`). | `effects/cardflow.go` (`effReveal`) | M4 (`RevealAllValid$`) |
| `effRepeat` and `effCharm`'s inner `Resolve` have no `Suspended()` guard and `effCharm` does not re-check `Suspended()` between modes, so a future asking sub there would ask N times. Measured corpus-unreachable today. | `effects/misc.go` (`effCharm`, `effRepeat`), `effects/registry.go` (`Resolve`) | M4 (add the `Suspended()` guards) |
...
| A sacrifice cost's `CARDNAME` is the source object id and source-less filters fail closed, but this does not make every CARDNAME-bearing cost playable: Exile/Discard/Return and other cost verbs remain unsupported, and mana abilities still use their separate incomplete payment path. | `effects/filter.go` (`MatchesObjectCtx`), `rules/cast.go` (`castable`, `sacAsk`) | M4 (remaining cost grammar and mana-ability payment) |
| The shared unless gate pays mana from the floating pool only (no mid-resolution tap-to-pay). An unless-cost the strict parser cannot price -- an unfolded `X`/`XX`, `PayEnergy<N>`, `Return<...>`, `LifeTotalHalfUp`, `DefinedCost_*`, dynamic SVars, `Y`/`Z` -- is a HARD DECLINE: the ask is posed and recorded but cannot be answered "pay". Exotic selector bindings remain unresolved. | `effects/unless.go` (`unlessProceed`, `poseUnlessAsk`), `rules/resolution.go` (`unless_pay` arm, `ParseUnlessCost`) | M4 (X-cost grammar, tap-to-pay, remaining selector bindings) |
...
| `Effect` registers real continuous effects only for `CantTarget`, `CantRegenerate` and the cost-modifier modes; `Triggers$` effects and every other `StaticAbilities$` mode stay a bare Note. Every other `Duration$` takes one of two WRONG lifetimes (UntilEOT on an instant/sorcery source or by name, else source-leaves) instead of its true expiry, and UntilYourNextTurn lasts a full turn too long. | `effects/misc.go` (`effEffect`, `effectUntilEOT`, `IsNextTurnDuration`), `rules/layers.go` (`restrictionApplies`, `AddContinuous`, `EndOfTurnCleanup`), `state/continuous.go` | M4 (Triggers$ effects, full Duration$ grammar, the other Effect-delivered grant keywords) |
| `DelayedTrigger` implements only `Mode$ Phase`; the other modes (`ChangesZone`, `SpellCast`, `ChangesController`, `DamageDone`, `AttackersDeclared`) record a Note only. `ValidPlayer$` and `RememberObjects$ Targeted` (vs `RememberedLKI`) capture are unmodelled. A registration whose source, SVar or controller is gone is skipped but never collected, so it is re-scanned forever. | `effects/misc.go` (`effDelayedTrigger`), `events/apply.go` (`DelayedRegister`, `DelayedPush`), `rules/trigger_match.go` (`checkDelayedTriggers`) | the remaining modes plus `ValidPlayer$`/`Targeted` capture |
...
| `ActivationLimit$` stays unenforced for a value that is neither a literal, an SVar reference nor an inline `Count$` (`resolveActivationLimit` reports ok=false); the compiled corpus carries no such shape today. | `rules/legal.go` (`activationLimitReached`, `resolveActivationLimit`) | none |
...
| (votepb1) The per-player vote CHOICE for the fixed-`Choices$` and `VoteCard$` ballots is the deterministic no-ask stand-in: every voter takes the ballot's first option, so it always wins unanimously. The tally lives on the resolution `Ctx` and is not carried across a downstream mid-resolution ask. `VoteMessage$`, `Secretly$` and `UpTo$` stay unread (the player ballot's secrecy is unconditional). | `effects/misc.go` (`effVote`, `effCardVote`, `voteWinner`), `effects/vote.go`, `rules/resolution.go` (the "vote" resume arm) | M4 (a real per-player ask for the `Choices$`/`VoteCard$` shapes; the tally across a suspension) |
| The three planechase verbs are registered no-ops with a Note: `api:Planeswalk` and `api:ChaosEnsues` record "no planar deck" because this build has no planar deck or planar zone (`Valid$ Plane` scans nothing), and their `Optional$`/`DontPlaneswalkAway$`/`Defined$`/`Remembered$` riders are unread. `T:Mode$ ChaosEnsues` is unregistered and silently inert. | `effects/planechase.go` (`effBlankLine`, `effPlaneswalk`, `effChaosEnsues`) | M4 (a planar-deck tier) |
| The monarch's end-step draw does not exist (Palace Jailer); only the designation itself is recorded. | `effects/misc.go` (`effBecomeMonarch`), `events/apply.go` (`MonarchChange`) | M4 |
...
| The R-9 no-host stand-in for an exact-`Origin$ Library` `ChangeZone`: the absent player's choice is the engine's deterministic first-`Min` pick (quantity-only) or a fail-to-find (stated-quality), never a real one. | `effects/zone.go` (`effSearchLibrary`, `applyLibrarySearch`) | none -- the no-ask host is the fuzz/test degradation contract |
...
| Only the exact single-origin spelling `Origin$ Library` enters the hidden search path; a compound origin containing Library (`Graveyard,Library` and kin) retains the object-movement path, since mixing a hidden library with public or sideboard candidates needs an origin-aware option API. | `effects/zone.go` (`effChangeZone`) | M4 (mixed-origin hidden searches) |
...
| The R-9 no-host stand-in for `Scry`/`Surveil`: a host that cannot ask keeps every card on top in its existing order (Scry's pile B empty, Surveil's graveyard pile empty), recorded as one `events.LibraryOrder` so the resolution completes rather than wedging. | `effects/cardflow.go` (`effLookAndArrange`) | M4 (a real host for every seat) |
...
| `Produced$ Any`/`Combo Any` resolve to colourless instead of asking, and `Combo X Y`/`Chosen` keep the executor's degenerate rune output rather than a selectable colour, so the tap gate can offer no real mana-colour choice. | `effects/misc.go` (`effMana`) | M4 mana-choice decisions |
...
| (fx20) `MatchesPlayerSpec` fails closed on any qualifier it does not list, so a trigger carrying `EnchantedBy`, `EnchantedController`, `Chosen`, `NonActive`, `descended`, `IsRemembered`, `TriggeredDefendingPlayer`, `counters_` or a compound/space form never fires (CR 603.4 says it should). Only `Active`, `life<OP><N>` and `isMonarch` evaluate. **Do NOT implement the missing qualifiers here.** | `effects/filter.go` (`MatchesPlayerSpec`), `rules/trigger_match.go` (`spellCastMatches`, `damageMatches`, `phaseMatches`), `rules/statics.go` (`actorMatches`), `rules/layers.go` (`restrictionActorMatches`) | M4 (a full player-spec grammar and the player-state machinery it needs) |
...
| (pw1) Planeswalker loyalty is narrow. Missing: combat damage to a walker (defenders are players only), `[-X]` dynamic costs (one-generic fallback, read only for gating), walker TOKENS (TokenCreate skips `Move`, so one enters at 0 loyalty and the SBA kills it), transform walkers, Effect-delivered `NumLoyaltyAct`. A `Loyalty:X` face is EXEMPT from the zero-loyalty SBA. | `events/apply.go` (`Move`), `rules/sba.go` (`planeswalkerZeroLoyalty`), `rules/mana.go` (`ParseCost`, `addCounterCost`), `rules/legal.go` (`isLoyaltyAbility`, `loyaltyAbilityLimit`), `rules/cast.go` | M4 (walker combat damage, [-X] cost grammar, walker tokens, transform walkers, Effect-delivered NumLoyaltyAct) |
...
| A Replicate (CR 702.55a) copy keeps the original's target: `MayChooseTarget$` gates only a Note, never a chooser. The count ask's max is conservative too: priced over the RAW composed cost (a ReduceCost modifier is unseen) and pool-only, so a Convoke contribution announced after the count cannot extend it. A bound of 0 degrades to a plain cast with a loud Note. | `effects/copy.go` (`effCopySpellAbility`'s MayChooseTarget arm), `rules/cast.go` (`replicateAsk`, `nonManaCastable`) | M4 (the copy-target task; the modifier-aware count bound; a convoke-extended count bound) |
...
| (ft1) Three foretell-adjacent shapes stay unimplemented: `Foretold$ True` as a ChangeZone param (a trigger marking an arbitrary exiled card foretold with a dynamic cost), the `Card.foretold` object predicate, and dynamic foretell-cost reduction. The {2} action is gated on "during your turn" only — deliberately no instant/sorcery-speed check. | `rules/legal.go` (the two offers), `rules/cast.go`, `rules/altcast.go` (`foretellCastAvailable`) | M4 (the three adjacent shapes) |
...
| (chosencopy1) `CanBeTargetedByTriggeredSpellAbility` is the target-SPEC half only — the protection/shroud/hexproof/CantTarget half is not modelled (the filter tier cannot reach rules' legality registry), so a protected creature still enters the choice pool. The `UnlessCost$` SVar fold is deliberately API-scoped to CopySpellAbility: the Counter family's `X`/`Y` spellings stay hard declines. | `effects/filter.go` (`triggeredSpellTargetSA`), `effects/unless.go` (`UnlessCostResolved`), `effects/copy.go` (`copyDefinedTargets`) | M4 (full canBeTargetedBy legality through the filter tier; the unscoped UnlessCost grammar) |
...
| (kw-class) Every Class level's granted body leans on its own primitive: a level whose `AddStaticAbility$`/`AddTrigger$`/`AddReplacementEffect$` names an unimplemented mode or effect is exactly as dead as that primitive. A `ClassBand$` band must NOT be folded into `IsPresent$`/`IsPresent2$` — the trigger gate reads those as a union and a granted replacement reads no `IsPresent2$` at all. | `cards/kw_class.go` (`addLevelGate`), `rules/class_level.go` (`classBandGateHolds`) | the granted bodies' own primitives |
...
| (api:TimeTravel) The Doctor Who action is real: every repetition re-enumerates the affected objects (owned suspended exile cards, then the controller's battlefield permanents with a TIME counter) in zone order and poses one KChoose add/remove/skip election per object, `Amount$` repetitions deep (The Tenth Doctor's 3). Each ask rides an immutable snapshot (`Decision.ResumeObjects`) with its cursor and repetition in two fields (`ResumeTarget`/`ResumeRound`), so an answer that drops a counter cannot shift the next object's cursor. The one stand-in is the R-9 no-host contract: a host that cannot ask declines every election. | `effects/time_travel.go` (`effTimeTravel`, `timeTravelObjects`), `rules/resolution.go` (the `time_travel` resume arm) | none -- the no-ask host is the fuzz/test degradation contract |
...
## The board clock's round number (exact, not an approximation)

`view.RoundOf` folds the ordered event stream, anchored on the starting player
and tracking eliminations, so an elimination alone never moves the round;
every log-bearing caller (host fan-out, `viewAt`, the seat view, mtgsim,
botbench, keywordbench) uses it and `roundOf` is only the snapshot fallback.
It never touches engine state. Revisit if a same-seat repeat turn (`AddTurn`)
ever lands.
