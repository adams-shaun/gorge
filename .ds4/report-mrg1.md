...
## Section 5 — merge of main into wt/cli-20260922T225139Z-244016f9 (DigUntil Aura bearer ticket)

The worktree was clean at the branch tip `eb3dfd2e` (`fix(effects): offer opponent
permanents for DigUntil Aura entry`) — no rebase/merge in flight; the daemon's failed
rebase had been rolled back. Integrated main as `git merge main` (rebase is forbidden
in this seat). Main's tip was `00147db0` (the sibling Dig closure,
cli-20260922T225139Z-230e9834).

### Conflicts and resolution

- `AGENTS.md` (the dig rows region): each side had deleted a DIFFERENT row.
  Main (b43ab0a9, the sibling Dig closure) deleted the `(dig1)` row; this branch
  (b67c4d3d, the approved DigUntil Aura bearer fix) deleted the `(diguntil1)` row,
  with its remaining debt (`RevealRandomOrder$`, the withheld params) carried in
  that commit's message per the frozen-register rule. Both deletions stand: the
  merged table has NEITHER row. Measured with the ratchet's own parsing: 71 data rows.
- `internal/testutil/agentsdoc_test.go`: each side had independently lowered
  `knownApproximationRows` to 72 (branch: no comment; main: a comment). Merged
  table measures 71, so the constant is set to 71 with a comment naming both
  deletions.
- `.ds4/report-mrg1.md`: auto-merged; this section appended.
- `decision/decision.go`, `effects/cardflow.go`, `rules/*`: auto-merged; both
  sides' engine changes retained unmodified.

### Commands and output

- `git status` before merge: clean on `wt/cli-20260922T225139Z-244016f9`.
- `git merge main`: conflicts in `AGENTS.md` and `internal/testutil/agentsdoc_test.go`;
  `decision/decision.go`, `effects/cardflow.go` auto-merged.
- `gofmt -l internal/testutil/agentsdoc_test.go` → clean.
- `awk` row count over the merged `AGENTS.md` table: 72 lines starting `| ` in the
  section = header + 71 data rows → matches `knownApproximationRows = 71`.
- `go test ./internal/testutil -run 'TestKnownApproximation' -v` →
  `--- PASS: TestKnownApproximationsOnlyShrinks`, `--- PASS: TestKnownApproximationRowsAreShort`,
  `ok github.com/adams-shaun/gorge/internal/testutil 0.001s` (exact count — no slack log).
- `go test ./rules -run 'TestNoTriggerModeIsRegistered|TestEveryDispatchedTriggerMode|TestEveryRepoDeck|TestEveryRepoDeckParams|CountHead' -v` →
  all PASS (TestEveryRepoDeckIsFullySupported 0.61s, TestEveryRepoDeckParamsAreRead 0.12s,
  both trigger-mode registry tests PASS, TestEveryRepoDeckCountHeadResolves PASS),
  `ok github.com/adams-shaun/gorge/rules 0.777s`. No ratchet table edits were needed:
  this branch registers no new trigger `Mode$` matcher, and the merged tree's
  `knownUnsupported` / `knownUnsupportedParams` / `knownUnmodelledCountHeads` tables
  are untouched by both sides (the stale `.ds4/scratch-mrg1.log` paramcensus
  `effDigUntil` failure does not reproduce — the branch's classification
  (f5d33462) carries over and main's sibling merge did not touch that scanner shape).
- Branch-fix sanity on the merged tree: `go test ./effects -run 'Dig'` → ok (0.640s);
  `go test ./rules -run 'Dig'` → ok (0.641s).
- `.cards` present (real corpus symlink) — runs are real, not vacuous skips.
- Final: `git commit` → merge `30fcb193`; `git status --short --branch` → clean.

### Notes / uncertainties

- The one judgement call: whether to keep main's `(diguntil1)` row (which still
  described the bearer approximation as open) or take the branch's deletion. The
  branch's fix is the reviewed change that CLOSES the bearer approximation, so its
  deletion stands; the row's residual debt (`RevealRandomOrder$`, withheld params)
  is documented in the branch commit's message (b67c4d3d), per the frozen-register
  rule that unclosed deviations live in the commit message, not the table.
