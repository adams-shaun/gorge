...
## Section 6 — current merge (main ae220e74 into wt/cli-20260922T225139Z-205fd0ae)

Started from a clean tree at `7efa1d0d`; no rebase/merge was in flight (the
daemon's failed rebase had been rolled back and this branch had already merged
main at `04ecdf2a`). Main had since advanced to `ae220e74` (the dig1
optional-choice closure `b43ab0a9`, the LIMITED-look/spectator closure
`e7a0d668` with its three re-pinned omniscient-decision tests, and their merge
chain), so the in-flight operation to finish was a fresh `git merge main`.

- Conflicts: `.ds4/report-mrg1.md` (both sides edit this shared report —
  resolved as the union: this worktree's earlier-round report above, main's
  Sections 1–5 verbatim, this Section 6 appended) and
  `internal/testutil/agentsdoc_test.go` (the `knownApproximationRows`
  constant: HEAD said 72 for this branch's hidden-origin deletion, main said
  71 for its limited-look deletion). `AGENTS.md` auto-merged — it retains all
  of main's row closures AND this branch's hidden-origin row deletion.
- `internal/testutil/agentsdoc_test.go`: resolved to **70** — the merged
  table's measured data rows (awk count over the `## Known approximations`
  section: 71 lines starting `| ` including the header = 70 data rows;
  main's 71 less this branch's one row deletion).
- No engine code was conflicted; `effects/zone.go` auto-merged with this
  branch's compound-origin fetch-player behaviour and main's limited-look
  changes combined.
...
go test ./rules -run 'TestNoTriggerModeIsRegistered|TestEveryDispatchedTriggerMode|TestEveryRepoDeck|TestEveryRepoDeckParams|CountHead|TestHeads'
ok  github.com/adams-shaun/gorge/rules  1.858s

go test ./effects -run 'Compound|LimitedLook|Dig'   # branch-fix + main's new suites on the merged tree
ok  github.com/adams-shaun/gorge/effects  0.611s

go test ./view -run 'Limited|Spectator'
ok  github.com/adams-shaun/gorge/view  0.015s

# post-commit confirmation
diff <(git show HEAD:.ds4/report-mrg1.md) .ds4/report-mrg1.md   # IDENTICAL
git merge-base HEAD main                                        # ae220e74 = main's tip
```

Merge commit: `70d2983c` (parents `7efa1d0d`, `ae220e74`). Working tree clean;
