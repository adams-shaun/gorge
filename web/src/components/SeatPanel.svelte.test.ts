...
describe('SeatPanel — the mulligan prompt names the starting player (rv2a)', () => {
  // This is the duplicate-deck, display-name contract produced by
  // rules.TestMulliganPromptNamesTheDisplayPlayerWithDuplicateDecks. The Go
  // integration test drives the real engine; this surface test proves the
  // wire prompt is not truncated or hidden by SeatPanel.
  const keepAskNamesStarter: Decision = {
    seq: 8, player: 1, kind: 'mulligan', min: 1, max: 1,
    prompt: 'Bob plays first. Keep your hand (put 0 cards on the bottom of your library) or take a mulligan?',
    options: [opt(0, 'keep', 'keep'), opt(1, 'mulligan', 'mulligan')],
  };

  it('shows the engine-produced display name for a duplicate-deck mulligan ask', () => {
