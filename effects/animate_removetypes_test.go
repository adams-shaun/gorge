...
func TestAnimateRemoveTypesRegistersTypeRemoval(t *testing.T) {
	...
	Resolve(h, c, sa(t, "DB$ Animate | Defined$ Self | RemoveTypes$ True | Duration$ UntilYourNextTurn"))
	if len(h.continuous) != 1 {
		t.Fatalf("continuous = %d, want one type-removal effect: %+v", len(h.continuous), h.continuous)
	}
	ce := h.continuous[0]
	if ce.Layer != state.LType || !ce.RemoveSubTypes || len(ce.AddTypes) != 0 {
		t.Fatalf("type grant = %+v; want LType RemoveSubTypes without additions", ce)
	}
}

func TestAnimateRemoveTypesBeforeAddedTypes(t *testing.T) {
	h, c := animateRemoveHost(t)
	Resolve(h, c, sa(t, "DB$ Animate | Defined$ Self | RemoveTypes$ True | Types$ Creature Alien | Duration$ Permanent"))
	if len(h.continuous) != 1 {
		t.Fatalf("continuous = %+v, want type effect", h.continuous)
	}
	ce := h.continuous[0]
	if !ce.RemoveSubTypes || len(ce.AddTypes) != 2 || ce.AddTypes[0] != "Creature" || ce.AddTypes[1] != "Alien" {
