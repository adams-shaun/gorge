...
// (-1: an out-of-range answer, i.e. a vote for nothing). The caster's own
// pick anchors "a choice you voted for"; a caster who is not among the
// voters (or voted for nothing) anchors nothing, so the same set is empty
// and every voting opponent lands in the diff set -- "a choice you didn't
// vote for" covers every choice when you made none.
func voteSameDiff(g *state.Game, caster state.PlayerID, voters []state.Target, picks []int) (same, diff []state.PlayerID) {
	casterPick := -1
...
// state, so replay takes the same branch and no recorded game changes. It is
// an over-approximation by design: any face with the mode, regardless of its
// TriggerZones$, turns the carrier on -- erring toward emitting is the safe
// direction.
