...
# exits — the exact command the deploy runs ahead of both demo servers, usable
# by hand or from cron. A no-op (zero Scryfall fetches) when the cache is
# already complete; exits non-zero if any name failed.
.PHONY: prewarm-art
prewarm-art:
