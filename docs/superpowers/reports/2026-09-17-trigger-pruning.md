...
scoped to the two approved batches; the later publication-rebase checkpoint
is recorded below.
...
opt-in conformance lane was not enabled. At this pre-rebase checkpoint, no
commit, push, merge or deployment had been performed.

## Publication-rebase checkpoint

The user subsequently requested a rebase onto origin and a remote push.
The optimization was committed, and all three local commits rebased cleanly
onto fetched `origin/main` at `e2657045c43a7d941398651a3162f62988802536`.
The target remains `origin/perf/hotspot-optimization-2026-09-17`; no merge
into main, PR creation or deployment was requested.

Upstream added botbench action coverage and grind mode and refreshed the
committed repro/host fixtures. The Board adapter now lives inside upstream's
`playMatchOnce` loop, retaining its collectors and wrapper. A build check
caught two parity-test calls missing the new `*actionCoverage` parameter;
adding `nil` at those call sites restored the test, without changing its
assertions or production behavior. The full botbench package passed.

`GOMAXPROCS=2 GOMEMLIMIT=5GiB go test -p=1 ./... -count=1` completed:
27 packages passed, including the full rules suite (403.320 s), and two
packages failed on exactly the three upstream-reproduced tests listed below.
The overall command therefore exited 1; this is not a green full-suite claim.

Post-rebase `make sim` verified 20/20 replays, with seed summaries and heads
identical to the pre-rebase simulator run. Go vet and generated-type checks
passed. A focused race run passed for `TestPlayMatchUsesBoardSeatWithViewParity`
and every `TestTriggerEligibility` test in botbench/rules. These checks used
Go 1.25.11, `GOMAXPROCS=2`, `GOMEMLIMIT=5GiB` and serial package workers.

The refreshed committed replay fixtures no longer reproduce their previous
divergences: the host and feedback packages passed. Three remaining failures
were independently reproduced from an exact `git archive e265704` source
snapshot, using the same corpus:

- `cmd/repro.TestReproOnFreshSnapshot`: the assertion expects 24 intents;
  both upstream and the candidate capture/replay 25, with head
  `775a4dcd17aaf606`. The replay itself verifies.
- `internal/archtest.TestTimeIsImportedOnlyByTheHost`: existing time imports
  in botbench and ledger.
- `internal/archtest.TestResumeStateOwnedOnlyByTheResolutionMachinery`:
  the same existing resume writers; diagnostic ordering and shifted source
  line numbers do not change that writer set.

These unrelated failures were not repaired or hidden. No fixture or golden
was regenerated by this branch. Web sources were unchanged and web lint was
not rerun; the opt-in known-red conformance lane remained disabled.

The performance measurements above still identify their original baseline
and candidate: no new timing claim is inferred from this publication rebase.
The rules/cards/effects/botpolicy/seat/state/events/replay trees are unchanged
from the measured candidate. Rebase verification artifacts are under
`/tmp/gorge-push-check.Y39KB6/`: `full-test.log`, `origin-failures.log`,
`race.log`, `sim.log`, `vet.log` and `gentypes.log`. The exact-upstream
diagnostic source snapshot is `/tmp/gorge-origin-check.NKGaZa/`.
