...
- **R-M4 Free-mulligan reading.** `Config.Mulligans = N` means each player may take up to N London mulligans; each forfeits one card (7 → 7−k after k mulligans); anyone who kept fewer than 7 then bottoms exactly `taken` cards (the London end-of-round bottoming). `Mulligans = 1` exercises one mulligan and a bottom of one — exactly the acceptance suite's need.
...
func TestABotThatMulligansDropsToSixAndBottomsOne(t *testing.T) {
