...
func TestCR733EarlyAbortSitesPreserveResources(t *testing.T) {
	requireCR601Audit(t, "abort-site resource and trigger audit")
	reg := testutil.CorpusRegistry(t)
	checked := 0
	for _, site := range []string{"sacrifice", "spell_mana", "spell_mana_after_choice", "activation_mana", "source_moved"} {
...
func TestCR733MiracleAbortAndTriggerControl(t *testing.T) {
	requireCR601Audit(t, "miracle entry, early return and cast-trigger audit")
	reg := testutil.CorpusRegistry(t)
	checked := 0
