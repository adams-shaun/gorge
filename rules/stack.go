...
func (e *Engine) payManaCast(pc *pendingCast, cost Cost) bool {
	...
	beforeSnow := e.G.Players[pc.player].Snow
	pay, ok := cost.resolveManaWith(before, beforeSnow, e.G.Players[pc.player].Life,
		e.payerGrantsPayLifeInsteadOfB(pc.player), pc.mayPlayIgnore)
	if !ok {
		return false
	}
	after, afterSnow, lifeSpent := pay.pool, pay.snow, pay.lifeSpent
	for i, letter := range manaLetters {
		...
		// The same snow-split emission payMana uses: a slot whose snow units
		// were spent emits the "S<colour>" Counter form so the parallel snow
		// tally moves with the pool through the same events the adds used.
		snowSpent := beforeSnow[i] - afterSnow[i]
		if snowSpent > 0 {
			if plain := spent - snowSpent; plain > 0 {
				e.emit(events.Event{Kind: events.ManaAdd, Player: pc.player, Counter: letter, Amount: -plain})
			}
			e.emit(events.Event{Kind: events.ManaAdd, Player: pc.player, Counter: "S" + letter, Amount: -snowSpent})
			continue
		}
