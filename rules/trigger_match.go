...
func (e *Engine) damageMatches(t cards.Trigger, source state.ObjID, ev events.Event) bool {
	...
	if v, ok := t.Params["ValidSource"]; ok {
		// The damage's source: the dealing creature during combat's assignment
		// loop (e.damaging, set alongside combatDamaging), the resolving spell
		// or ability while it is the stack top (damageSource) otherwise. During
		// combat the stack is USUALLY empty, but not always -- the
		// between-passes priority round (CR 510.3/4) can leave a first-strike
		// trigger on the stack while the regular pass deals (measured:
		// TestUmezawasJitteGainsChargeCountersPerDamageStep's bearer deals in
		// both passes with the first pass's trigger unresolved on the stack) --
		// so combat damage must prefer e.damaging over the stack top, or every
		// ValidSource$ CombatDamage$ trigger (Umezawa's Jitte's
		// ValidSource$ Creature.EquippedBy among them) goes dead for the second
		// pass.
		src := e.damaging
		if src == 0 {
			src = e.damageSource()
