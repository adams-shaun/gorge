...
func (e *Engine) handleChoose(d *decision.Decision, in decision.Intent) {
	...
	// reveal-optional, defined-library-optional, and future siblings). Dispatch
	// by role rather than an allowlist: the asking effect already recorded the
	// exact SA and answer interpretation in e.resume, while cast/cleanup flows
	// never do (the kinds that bypass this check -- madness, optional
	// triggers, their Cost$ windows -- are answered through KTriggerOptional
	// and the chooseTriggeredCost/chooseCumulative arms, never KChoose). This
	// is the structural guard against silently dropping the next KChoose-based
	// primitive merely because its string was not added here. An empty chosen
	// slice is the legitimate "fail to find" / Optional-decline answer.
	if e.resume != nil {
		rp := e.resume
		e.resume = nil
