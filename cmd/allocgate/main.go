...
func main() {
	...
	case *changed:
		...
		if len(touched) == 0 {
			if len(staged) > 0 {
				...
				fmt.Fprintln(os.Stderr,
					"allocgate: -changed selects packages from the staged set (git diff --cached --name-only); nothing is staged, so no packages were measured -- this is an empty selection, not a measurement. Stage the files you changed, or name packages explicitly, e.g. go run ./cmd/allocgate ./rules")
			}
			return
		}
		...
		if len(selected) == 0 {
			...
			fmt.Fprintln(os.Stderr,
				"allocgate: -changed selects packages from the staged set (git diff --cached --name-only); none of the staged files is inside a Go package, so no packages were measured -- this is an empty selection, not a measurement. Name packages explicitly, e.g. go run ./cmd/allocgate ./rules")
			return
		}
	case *all:
		for _, p := range pkgs {
