...
func TestConstructedDefaultIsByteIdentical(t *testing.T) {
	...
	// re-measured by VoteCard$'s per-voter decision: Council's Judgment in
	// death-n-taxes now asks each voter and the bot selects the highest-worth
	// opposing permanent instead of ballot order's first (reverting
	// effects/misc.go returns 5/15). It is also covered by the
